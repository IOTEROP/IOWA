...
#define CBOR_ADD_INFO_MASK             0x1F // 00011111
#define CBOR_ADD_INFO_1_BYTE           24 // next byte is value with uint8_t format (between 32 and 255)
#define CBOR_ADD_INFO_2_BYTES          25 // next bytes is value with uint16_t or IEEE 754 Half-Precision Float format
#define CBOR_ADD_INFO_4_BYTES          26 // next bytes is value with uint32_t or IEEE 754 Single-Precision Float format
#define CBOR_ADD_INFO_8_BYTES          27 // next bytes is value with uint64_t or IEEE 754 Double-Precision Float format
...
#define CBOR_MAJOR_TYPE_MASK           0xE0 // 111000000
#define CBOR_MAJOR_TYPE_BIT_SHIFT      5 // b7b6b5b4b3b2b1b0 => 111000000
...
(uint8_t)(((MAJOR_TYPE<<CBOR_MAJOR_TYPE_BIT_SHIFT)&CBOR_MAJOR_TYPE_MASK)+(ADD_INFO&CBOR_ADD_INFO_MASK))
...
// - resTypeCb: resource type callback. This can be nil.
// - userDataP: user data passed to resTypeCb. This can be nil.
// Note: if resTypeCb is not set, data type will be undefined
...
// Convert OPAQUE buffer into LwM2M data.
// Returned value: IOWA_COAP_NO_ERROR in case of success or an error status.
// Parameters:
// - baseUriP: URI of the data. Can not be at instance nor object level. Can not be the Root path.
// - bufferP, bufferLength: payload to deserialize.
...
// - userDataP: user data passed to resTypeCb. This can be nil.
// Note:
// - if resTypeCb is not set, data type can be string, opaque, integer, float or boolean
// - Support Half float, decimal fraction, null and undefined value
iowa_status_t cborDeserialize(iowa_lwm2m_uri_t *baseUriP, uint8_t *bufferP, size_t bufferLength, iowa_lwm2m_data_t **dataP, size_t *dataCountP, data_resource_type_callback_t resTypeCb, void *userDataP);
...
// - userDataP: user data passed to resTypeCb. This can be nil.
// Note:
// - if resTypeCb is not set, data type can be string, opaque, float, boolean, object link
// - Support floating timestamp, URI only
// - Handle excessive space
iowa_status_t senmlJsonDeserialize(uint8_t *buffer, size_t bufferLength, iowa_lwm2m_data_t **dataP, size_t *dataCountP, data_resource_type_callback_t resTypeCb, void *userDataP);
...
// - userDataP: user data passed to resTypeCb. This can be nil.
// Note:
// - if resTypeCb is not set, data type can be string, opaque, integer, float, boolean, object link
// - Support floating timestamp, URI only
// - Support Half float, decimal fraction, null and undefined value
// - Support Indefinite size of array, map, text string and byte string
iowa_status_t senmlCborDeserialize(uint8_t *buffer, size_t bufferLength, iowa_lwm2m_data_t **dataP, size_t *dataCountP, data_resource_type_callback_t resTypeCb, void *userDataP);
...
iowa_status_t jsonDeserialize(iowa_lwm2m_uri_t *baseUriP, uint8_t *bufferP, size_t bufferLength, iowa_lwm2m_data_t **dataP, size_t *dataCountP, data_resource_type_callback_t resTypeCb, void *userDataP);
