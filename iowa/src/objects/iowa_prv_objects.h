...
// Parameters:
// - contextP: returned by iowa_init().
// Note: IOWA_DEVICE_SUPPORT_RSC_FACTORY_RESET must be defined
void objectDeviceFactoryReset(iowa_context_t contextP);

/* Firmware object */

iowa_status_t objectFirmwareClose(iowa_context_t contextP);
...
iowa_status_t objectOscoreCreate(iowa_context_t contextP, uint16_t id);
iowa_status_t objectOscoreRemove(iowa_context_t contextP, uint16_t id);
