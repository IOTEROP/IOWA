...
#define IOWA_COAP_OBSERVE_REQUEST_NEW     0  // coap option's value for new observation
#define IOWA_COAP_OBSERVE_REQUEST_CANCEL  1  // coap option's value for cancellation

#define COAP_DEFAULT_PORT        "5683"
#define COAP_DEFAULT_PORT_SEC    "5684"
#define COAP_DEFAULT_PORT_WS     "80"
#define COAP_DEFAULT_PORT_WS_SEC "443"

#define COAP_LORAWAN_PORT_MIN 1
#define COAP_LORAWAN_PORT_MAX 255

#define COAP_CODE_CLASS_MASK  0xE0

#define COAP_CLASS_REQUEST      0x00
#define COAP_CLASS_SUCCESS      0x40
#define COAP_CLASS_CLIENT_ERROR 0x80
#define COAP_CLASS_SERVER_ERROR 0xA0
#define COAP_CLASS_SIGNALING    0xE0

#define COAP_IS_REQUEST(code)   (((code) & COAP_CODE_CLASS_MASK) == COAP_CLASS_REQUEST)
#define COAP_IS_SUCCESS(code)   (((code) & COAP_CODE_CLASS_MASK) == COAP_CLASS_SUCCESS)
#define COAP_IS_ERROR(code)     ((((code) & COAP_CODE_CLASS_MASK) == COAP_CLASS_CLIENT_ERROR) || (((code) & COAP_CODE_CLASS_MASK) == COAP_CLASS_SERVER_ERROR))
#define COAP_IS_SIGNALING(code) (((code) & COAP_CODE_CLASS_MASK) == COAP_CLASS_SIGNALING)
...
typedef struct _block_payload_t block_payload_t;
...
#define IOWA_BUFFER_EMPTY (iowa_buffer_t){NULL, NULL, 0}
...
// Handle iowa_clock_reset() at CoAP level.
// Returned value: none.
// Parameters:
// - contextP: returned by iowa_init().
void coapClockReset(iowa_context_t contextP);
...
// Get the MAX_TRANSMIT_WAIT of a CoAP peer.
// Returned value: The MAX_TRANSMIT_WAIT, INT32_MAX if the peer is a Stream one, or -1 in case of error.
// Parameters:
// - peerP: a CoAP peer.
int32_t coapPeerGetMaxTxWait(iowa_coap_peer_t *peerP);

// Get the EXCHANGE_LIFETIME of a CoAP peer.
// Returned value: The EXCHANGE_LIFETIME, INT32_MAX if the peer is a Stream one, or 0 in case of error.
// Parameters:
// - peerP: a CoAP peer.
int32_t coapPeerGetExchangeLifetime(iowa_coap_peer_t *peerP);

// Generate a new unused CoAP token.
// Returned value: '0' in case of success or an error code in the form of a CoAP code.
// Parameters:
// - peerP: the CoAP peer to send the message to.
// - lengthP: OUT. the length in bytes of 'tokenP'.
// - tokenP: OUT. the generated token.
...
// - peerP: the CoAP peer which sent the message.
// - messageP: the received CoAP message.
// Note: CoAP Reset is not available for TCP connection
void coapSendReset(iowa_context_t contextP,
                   iowa_coap_peer_t *peerP,
                   iowa_coap_message_t *messageP);

// Request the full message
// Returned value: none.
// Parameters:
// - peerP: the CoAP peer which sent the message.
// - messageP: the received CoAP message.
// - resultCallback: the callback called when the full message is received, or failed. This can be nil.
// - userData: past as parameter to resultCallback. This can be nil.
iowa_status_t coapRequestAll(iowa_context_t contextP,
                             iowa_coap_peer_t *peerP,
                             iowa_coap_message_t *messageP,
                             coap_message_callback_t resultCallback,
                             void *userData);
...
// - internalResultCb: inform if the result callback is an internal callback.
// - resultCb: The callback to call when the next block is received.
// - userData: past as parameter to resultCallback. This can be nil.
uint8_t coapBlockRequestNext(iowa_context_t contextP,
                             iowa_coap_peer_t *peerP,
                             iowa_coap_message_t *messageP,
                             bool internalResultCb,
