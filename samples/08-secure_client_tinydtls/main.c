...
// PSK credentials
#define KEY_IDENTITY SAMPLE_PSK_IDENTITY
#define KEY_VALUE SAMPLE_PSK_KEY
...
iowa_status_t iowa_system_security_data(const uint8_t *peerIdentity,
                                        ...
                                        void *userDataP)
{
    uint8_t keyIdentity[] = KEY_IDENTITY;
    uint8_t keyValue[] = KEY_VALUE;
    size_t keyIdentityLength;
    size_t keyValueLength;

    (void)userDataP;

    if (0 == sizeof(keyIdentity) || 0 == sizeof(keyValue))
    {
        fprintf(stderr, "\r\n\n-------------------------\r\n\n"
    ...
    keyIdentityLength = strlen(keyIdentity);
    ...
    case IOWA_SEC_READ:
        ...
        {
            securityDataP->protocol.pskData.identity = (uint8_t *)malloc(keyIdentityLength + 1); /* zstr */
            if (NULL == securityDataP->protocol.pskData.identity)
            {
                fprintf(stderr, "Memory allocation failure.\r\n");
                return IOWA_COAP_501_NOT_IMPLEMENTED;
            }
            strcpy(securityDataP->protocol.pskData.identity, keyIdentity);
         ...
         char *argv[])
{
    ...
    printf("This a simple LwM2M Client using tinydtls to secure its exchanges with the LwM2M Server.\r\n\n");
    ...
    devInfo.modelNumber = "secure_client_tinydtls";

    // Configure the LwM2M Client
    result = iowa_client_configure(iowaH, IOWA_DEVICE_NAME , &devInfo, NULL);
