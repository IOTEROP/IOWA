         ...
         char *argv[])
{
    ...
    printf("This a simple bootstrap LwM2M Client.\r\n\n");
    ...
    devInfo.modelNumber = "bootstrap_client";
    ...
    result = iowa_client_add_bootstrap_server(iowaH, SERVER_URI, IOWA_SEC_NONE);
    if (result != IOWA_COAP_NO_ERROR)
    {
        fprintf(stderr, "Adding a bootstrap server failed (%u.%02u).\r\n", (result & 0xFF) >> 5, (result & 0x1F));
        goto cleanup;
    }

    printf("Starting a Client. Connecting to the LWM2M bootstrap server.\r\n\n");
